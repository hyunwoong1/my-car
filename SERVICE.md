...
- 한 사용자가 차종이 다른 차량을 여러 대 가질 수 있으므로, 이전 대화나 세션에서 확인된 차종을 다른 질문에 넘겨짚어 그대로 적용하지 않는다 — 지금 질문에서 확인되지 않았다면 다시 묻는다
