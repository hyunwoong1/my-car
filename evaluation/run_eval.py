...
Supervisor의 단기 기억(체크포인트)도 이제 SQLite 파일로 영속화되므로, 같은 이유로
evaluation/checkpoints_test.sqlite라는 별도 테스트 파일을 쓰고 매 실행 전 지운다. 그렇지 않으면
케이스마다 고정된 thread_id(f"eval-{id}")에 이전 라운드의 대화 기록이 남아 오염될 수 있다.
"""
import json
import os
import sys
...
for path in (TEST_DB_PATH, TEST_CHECKPOINT_PATH):
